        ...
        if (OK)													//only enters loop if valid file names are entered from getnames
        {
            ...
            while(word != null)									//while the string is not null
            {
                ...
                while(inline.hasMoreTokens())					//while loop checking if more tokens are available to check
                {
                    ...
                    boolean negative = false;					//creates boolean called negative and sets to false
                    if (Character.isDigit(first))
                    {
                        ...
                        if (isInt(num))	//checks if num as a valid number
                        {
                            ...
                            System.out.println("Found number: " + temp);
                    ...
                    while(!Character.isLetterOrDigit(first) && word.length() > 1)	//while loop for when the first character is neither a number or a digit
                    {
                        if (Character.isDigit(word.charAt(1)))	//checks if the character after first is a number
                        {
                            ...
                            else
                            {
                                ...
                                System.out.println("Number: " + num);		//debugging
                                if (word.length() > 0)
                                {
                                    first = word.charAt(0);		//moves first character to the first character in the new string
                                }
            ...
            int u = 0;
            int unique = 0;
            while (u < mywords.length && mywords[u].getWord() != "")
            {
                if (mywords[u].getCount() == 1)
                {
                    unique++;
                }
                u++;	//increments c by one
            }
            outfile.println("Number of unique words:\t" + unique);
...
class IOFile extends Program2		//IOFile class header
{
    ...
    {
        name = name+exten;		//combines name and extension into one string
        String newname = name + exten;		//puts string name and .bak extension into string named newname
    ...
    {
        File test = new File(name);				//creates new file called test with name of string
        return test.getAbsolutePath();			//returns absolute path of name
    }
    ...
    static String promptInput(String name)			//functioning for validating input files
    {
        ...
        do
        {
            if(!line.isEmpty())		//checks to make sure line isn't empty
            {
                ...
                {
                    System.out.println("Found file, moving on");		//prints message for file found
                    result = IOFile.FilePath(line);						//gets path and stores it into result
                }
                else
                {
        ...
        if (result.isEmpty())
        {
            System.out.println("Terminating");		//lets user know they are terminating due to no entry
        }
    ...
    static String promptOutput(String name)		//function for validating input files
    {
        ...
        do
        {
            if(!line.isEmpty())
            {
                ...
                if (!outexist)
                {
                    System.out.println("File doesn't exist, moving on");		//message for output not existing
                    result = IOFile.FilePath(line);								//stores path in result
                }
                else
                {

                    System.out.println("File already exists");		//message for file already existing
                }
            }
            ...
            if (outexist)		//result not initialized
            {
                ...
                {
                    IOFile.FileBackup(IOFile.FileName(temp), IOFile.FileExtension(temp));	//backs up file if 'o' is entered
                    outexist = IOFile.FileExist(temp);										//double checks existence
                    result = IOFile.FilePath(temp);											//stores path in result
                }
            }
        ...
        if (result.isEmpty())
        {
            System.out.println("No entry, terminating");	//terminating message
        }
        ...
        switch (args.length)
        {
            case 0:
                ...
                if (ioname[0].isEmpty())		//checks if ioname is empty
                {
                    ...
                    System.out.println("Terminating");	//terminating message
                }
                else
                {
